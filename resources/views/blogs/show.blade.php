@php
    $usingSidebar = auth()->check();
    $outerClasses = $usingSidebar ? 'px-6 py-6 bg-gray-50 min-h-full' : 'min-h-screen bg-gray-50 py-12';
    $innerClasses = $usingSidebar ? 'max-w-4xl mx-auto space-y-8' : 'max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8';
@endphp

@extends($usingSidebar ? 'layouts.sidebar' : 'layouts.app')

@section('title', $blog->title . ' - FixIt Blog')

@if($usingSidebar)
    @section('page-title', $blog->title)
    @section('page-description', 'Expert insights and updates from FixIt Solutions')
@endif
